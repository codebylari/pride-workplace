...
export function CompanySidebar({ showSidebar, setShowSidebar }: CompanySidebarProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50" onClick={() => setShowSidebar(false)}>
      ...
      >
        {/* Profile Section */}
        <div className="p-3 flex items-center gap-2 border-b border-white/20 flex-shrink-0">
          <div className="w-12 h-12 rounded-full bg-gray-300 overflow-hidden border-2 border-white/30 flex-shrink-0">
            {logoUrl ? (
              <img src={logoUrl} alt={`Logo ${companyName}`} className="w-full h-full object-cover" loading="lazy" />
            ) : (
              <div className="w-full h-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center text-base font-bold text-white">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-white/80">empresa</p>
          </div>
        </div>
        ...
        <div className="p-2 border-t border-white/20 flex-shrink-0">
          <button 
            ...
            className="w-full flex items-center gap-2 px-3 py-2 hover:bg-white/10 rounded-lg transition text-left text-red-400"
          >
